...
import cn.tellsea.dto.RedisInfo;//dto.RedisInfo;
...
public class RedisServiceImpl implements RedisService {
    ...
    @Override
    public Boolean hmset(String key, JSONObject value) {
        Map<byte[], byte[]> data =new HashMap<>();
//循环转换
        Iterator it =value.entrySet().iterator();
        while (it.hasNext()) {
            Map.Entry<String, Object> entry = (Map.Entry<String, Object>) it.next();
            data.put(entry.getKey().getBytes(), entry.getValue().toString().getBytes());
        }
         execute().hMSet(key.getBytes(), data);
         return true;
    }
